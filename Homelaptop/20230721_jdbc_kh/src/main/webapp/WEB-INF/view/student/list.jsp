...
<body>
	...
	<table border="1">
		...
		<tr>
			...
			<td><a
				href="<%=request.getContextPath()%>/student/get?sno=<%=vo.getStudentNo() %>"><%=vo.getStudentName() %></a></td>
...
if(startPageNum != 1){
	%>
		<a href="<%=request.getContextPath() %>/student/list?pageNo=<%=startPageNum %>&searchWord=<%=searchWord %>"><span>이전</span></a>
		,
		<%
